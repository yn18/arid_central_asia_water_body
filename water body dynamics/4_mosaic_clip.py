...
def mosaic_clip(tifs, mask, year, pathout):
    ...
    out_data = pathout + '/' + str(year) + '_hp.tif'

    driver = gdal.GetDriverByName('GTiff')
    driver.CreateCopy(out_data, ds_mosaic, strict=1, options=["TILED=YES", "COMPRESS=LZW"])
...
def main():
    ...
    if comm_rank == 0:
        ...
        datas = [x+1990 for x in range(10)]
    ...
    for year in mpi_data_divide:
        tile_datas = []
        for tile in tiles:
            tile_data = args.input + '/' + tile[0:4] + '/' + tile[-4:] + '/' + tile + '/' + tile + '_' + str(year) + '_o.tif'
