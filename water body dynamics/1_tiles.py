...
def retiles(tasks, tile, tif, year, pathin, pathout):
    ...
    out_data = pathout_1 + '/' + tile + '_' + year + '.tif'
...
def main():
    ...
    for data in mpi_datas_divide:
        year = data.split('/')[-1].split('.')[0].split('_')[1]

        for tile in tiles:

            retiles(args.tasks, tile, data, year, args.input, args.output)
